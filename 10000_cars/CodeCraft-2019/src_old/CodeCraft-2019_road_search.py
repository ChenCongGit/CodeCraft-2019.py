...
#def get_crowd_road(roadInfor, carInfor, cars, roads, crosses, answer_dict, current_time):
    

#    return crowd_roads_list
    
# 调用判题器获得死锁路口名称，如果当前时刻存在死锁，返回死锁路口id，如果不存在死锁返回None
#def get_loc_cross(roadInfor, carInfor, cars, roads, crosses, answer_list, t):

    
#   return loc_cross

# # 调用判题器获取当前时刻所有车辆的位置，返回当前车辆位置的字典
#def current_car_dict = get_current_info(roadInfor, carInfor, cars, roads, crosses, answer_list, t):

    
#    return current_car_dict
...
def init_schedule(min_dist_dict, cars, roads):
    ...
    for i in range(len(cars)):
        ...
        if car_plantime == 1:
            ...
            answer_dict[car_id] = [start_time, [min_dist_path_by_road[0]]]
            CARDICT[car_id].simulateInit(start_time, answer_dict[car_id][1])
            CROSSDICT[CARDICT[car_id].__from__()].carportInitial(CARDICT[car_id].__planTime__(), car_id)
            final_answer_dict[car_id]=[start_time, [min_dist_path_by_road[0]]]

        if len(init_go_cars) >= init_cars:
            break
    
    print(len(answer_dict))
    return init_go_cars, answer_dict


# 后续连续发车，每次发车时根据当前时刻路况修改图权值
def schedule(min_dist_dict, cars, roads, crosses, init_go_cars, answer_dict):
    # 剩余车辆
    print(CARDICT[11000].__state__())
    
    seq_cars = [i for i in cars if i not in init_go_cars]
    
    # 用一个字典存放最终要得到的answer结果，用于生成answer.txt，字典不可删除
    final_answer_dict = copy.deepcopy(answer_dict)
    ...
    # 上一时刻我们希望发出但是没有发出的车的id列表
    expect_but_not_go_cars_list = []
    ...
    for t in range(1,10000):
        ...
        # 调用判题器，我们希望出发400辆车，但是判题器实际在1时刻只能出发385辆，所以在answer中删除实际没出发的车
        roadInfor,carInfor = realtimetraffic()
        expect_but_not_go_cars_list = list(answer_dict.keys() - carInfor.keys())
        for i in expect_but_not_go_cars_list:
            del answer_dict[i]
            del final_answer_dict[i]
        ...
        for key, value in roadInfor.items():
            if roadInfor[key][2] > 0.6:
                ...
                crowd_roads.append(cross_road_dict[(roadInfor[key][0], roadInfor[key][1])])
            if roadInfor[key][3] > 0.6:
                crowd_roads_from.append(roadInfor[key][1])
                crowd_roads_to.append(roadInfor[key][0])
                crowd_roads.append(cross_road_dict[(roadInfor[key][1], roadInfor[key][0])])
        ...
        if cars == []:
            ...
            for car in cars:
                ...
                if CARDICT[car_id].__state__() == 1 or CARDICT[car_id].__state__() == 2:
                    ...
                    if (min_dist_path[0] in crowd_roads_from) and (min_dist_path[1] in crowd_roads_to):
                        ...
                        # 如果当前时刻车辆与上一时刻在同一条路上，则保留上一次的answer不变
                        if current_car_dict[car_id][1] == last_car_location[car_id][1]:
                            ...
                            for road in crosses[current_car_dict[car_id][1]-1][1:4]:
                                ...
                                # 除去来的道路和-1
                                if (road != -1) and (road != answer_dict[car_id][1][-1]):

                                    # 如果道路为双向车道，或者该点是起点，则该道路可选
                            ...
                            index = index(min(choose_road_speed_list))
                            next_road = choose_road_list[index]
                            CARDICT[car_id].route.replace()

                            CARDICT[car_id].simulateInit(current_time, answer_dict[car_id][1])
                            CROSSDICT[CARDICT[car_id].__from__()].carportInitial(CARDICT[car_id].__planTime__(), car_id)


                    else:
                        answer_dict[car_id][1].append(min_dist_path_by_road[0])
                        CARDICT[car_id].simulateInit(current_time, answer_dict[car_id][1])
                        CROSSDICT[CARDICT[car_id].__from__()].carportInitial(CARDICT[car_id].__planTime__(), car_id)
                    ...
                    if (car_plantime <= t)and(car_from-1 not in crowd_roads_from)and(car_from-1 not in crowd_roads_to)and(cross_already_go_cars[car_from-1] <= 30):
                        start_time = t
...
def main():
    ...
    init_go_cars, answer_dict = init_schedule(min_dist_dict, cars, roads)
    #final_answer_dict = schedule(min_dist_dict, cars, roads, crosses, init_go_cars, answer_dict)
    
    
    with open(answer_path, 'a') as wf:
        for key, value in final_answer_dict.items():
            wf.write('(' + str(key) + ',') 
            wf.write(str(value[0]) + ',')  
            for road_id in range(len(value[1]) - 1):
                wf.write(str(value[1][road_id]) + ',')

            wf.write(str(value[1][-1]) + ')\r\n')
