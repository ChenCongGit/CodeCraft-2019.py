...
def schedule(min_dist_dict, cars, roads, crosses, answer_path, init_go_cars):
    ...
    while True:
        ...
        for key, value in roadInfor.items():
            ...
            if  CARDISTRIBUTION[1]>1000: #CARDISTRIBUTION[1] 为判题器判定的在路上的行驶车辆数
            ...
            elif  900<=CARDISTRIBUTION[1]<=1000:#TIME[0] > 400 and
            ...
            elif 400<=CARDISTRIBUTION[1]<900:

                if roadInfor[key][2] > 0.6 or ROADDICT[key].channel_ <= 2 and roadInfor[key][2] > 0.4:
                    will_go_crowd_roads_from.append(roadInfor[key][0])
                    will_go_crowd_roads_to.append(roadInfor[key][1])

                if roadInfor[key][3] > 0.6 or ROADDICT[key].channel_ <= 2 and roadInfor[key][3] > 0.4:
                    will_go_crowd_roads_from.append(roadInfor[key][1])
                    will_go_crowd_roads_to.append(roadInfor[key][0])
                ...
                if roadInfor[key][2] > 0.75 or ROADDICT[key].channel_ <= 2 and roadInfor[key][2] > 0.45:
                    will_go_crowd_roads_from.append(roadInfor[key][0])
                    will_go_crowd_roads_to.append(roadInfor[key][1])

                if roadInfor[key][3] > 0.75 or ROADDICT[key].channel_ <= 2 and roadInfor[key][3] > 0.45:
                    will_go_crowd_roads_from.append(roadInfor[key][1])
                    will_go_crowd_roads_to.append(roadInfor[key][0])
                ...
                if (car.__state__() == 1 or car.__state__() == 2) and car. __nextCrossId__() != car_to:
                    ...
                    if car.planTime_  == -1:
                        ...
                        if (car_plan_time <= TIME[0]) and (car_from not in will_go_crowd_roads_from) and (
                            car_from not in will_go_crowd_roads_to) and ((cross_already_go_cars[car_from - 1] <= 1)
                                or ((cross_already_go_cars[car_from - 1] <= 20) and (on_road_num < 1000) and TIME[0]>350)):
                            will_go_num += 1
        ...
        if CARDISTRIBUTION[2] == CARNAMESPACE.__len__() or (on_road_num <= 400 and TIME[0] > 550 and not_go_num == 0):# or on_road_num <= 300:
            break_time = TIME[0]
            break
        else:
            TIME[0] += 1
...
def main():
    ...
    car_path = "/home/liushixin/AI/chendali/Chencong/huawei2019/me/SDK/SDK_python/CodeCraft-2019/config/car.txt"#$sys.argv[1]
    road_path ="/home/liushixin/AI/chendali/Chencong/huawei2019/me/SDK/SDK_python/CodeCraft-2019/config/road.txt" #sys.argv[2]
    cross_path = "/home/liushixin/AI/chendali/Chencong/huawei2019/me/SDK/SDK_python/CodeCraft-2019/config/cross.txt"#sys.argv[3]
    answer_path = "/home/liushixin/AI/chendali/Chencong/huawei2019/me/SDK/SDK_python/CodeCraft-2019/config/answer.txt"#sys.argv[4]
